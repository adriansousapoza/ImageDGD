...
**TSNE perplexity warning.** The cuML branch's `TSNE(n_components=n_components, perplexity=30, max_iter=1000)` relies on cuML's default `n_neighbors=90`, which sits exactly on `3 * perplexity`. cuML's internal check appears to require strictly more than that boundary (confirmed empirically: the default 90 triggers the warning; an explicit 91 does not). Fix: pass `n_neighbors=3 * 30 + 1` explicitly, **cuML branch only** — `sklearn.manifold.TSNE` has no `n_neighbors` parameter at all (confirmed against its signature: `perplexity`, `early_exaggeration`, `learning_rate`, `max_iter`, `n_iter_without_progress`, `min_grad_norm`, `metric`, `metric_params`, `init`, `verbose`, `random_state`, `method`, `angle`, `n_jobs` — no `n_neighbors`), so adding it there would raise `TypeError`. The sklearn (CPU fallback) branch is untouched; the warning is cuML-specific.
